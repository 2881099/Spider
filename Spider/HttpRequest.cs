	...
	protected void receive(Stream stream, int redirections, Uri uri, Encoding encode) {
		...
		while (overs > 0) {
			...
			if (_response == null) {
				...
				idx = findBytes(bytes, new byte[] { 13, 10, 13, 10 }, 0);
				if (idx != -1) {
					headStream.Close();
			...
			if (_response != null) {
				if (string.Compare(_response.TransferEncoding, "chunked") == 0) {
					...
					do {
						isContinue = false;
						idx = HttpRequest.findBytes(chunks, new byte[] { 13, 10 }, lidx);
						if (idx != -1) {
							...
							if (int.TryParse(chu[0], System.Globalization.NumberStyles.HexNumber, null, out chunkSize)) {
								...
								if (esize >= chunkSize + 2) {
									...
									if (chunkStream.Length == 5) {
										idx = HttpRequest.findBytes(chunks, new byte[] { 48, 13, 10, 13, 10 }, 0);
										if (idx != 0) {
											isBreak = true;
											break;
										}
...
public partial class HttpResponse {
	...
	public byte[] GetStream() {
		switch (_contentEncoding.ToLower()) {
			case "gzip":
				return HttpRequest.GZip.Decompress(_stream);
			case "deflate":
				return HttpRequest.Deflate.Decompress(_stream);
			default:
				return _stream;
		}
	}
	public void Save(string filename) {
		using (FileStream fs = new FileStream(filename, FileMode.Create)) {
			switch (_contentEncoding.ToLower()) {
				case "gzip":
					byte[] gzip = HttpRequest.GZip.Decompress(_stream);
					fs.Write(gzip, 0, gzip.Length);
					break;
				case "deflate":
					byte[] deflate = HttpRequest.Deflate.Decompress(_stream);
	...
	public string Xml {
		get {
			if (_xml == null) {
				switch (_contentEncoding.ToLower()) {
					case "gzip":
						_xml = Encoding.GetEncoding(_charset).GetString(HttpRequest.GZip.Decompress(_stream));
						break;
					case "deflate":
						_xml = Encoding.GetEncoding(_charset).GetString(HttpRequest.Deflate.Decompress(_stream));
						break;
					default:
